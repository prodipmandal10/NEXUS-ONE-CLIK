...
# পুরনো স্ক্রিন session থাকলে বন্ধ করুন
screen -S NEX -X quit 2>/dev/null

# নতুন screen session তৈরি করুন bash কে login shell হিসেবে চালিয়ে
screen -dmS NEX bash -lic "nexus-network start --node-id $NODE_ID; exec bash"
